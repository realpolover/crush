...
<button
    ...
    {disabled}
    on:click
    class="inline-flex items-center justify-center rounded-xl font-semibold transition-all duration-150 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 {variants[variant]} {sizes[size]} {className}"
    {...$$restProps}
>
    <slot />
</button>
